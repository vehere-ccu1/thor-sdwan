...
-- View: sites with account and group (Master-Owner, Created-By, GroupName, Created-At)
-- Use account's master_owner_user_id when site's is null/nil so Master Owner column is always populated.
DROP TABLE IF EXISTS sdwan_cms.sites_with_account;
CREATE VIEW sdwan_cms.sites_with_account AS
SELECT
    oa.id,
    oa.account_id,
    oa.group_id,
...
FROM (
    ...
    FROM (SELECT * FROM sdwan_cms.sites FINAL) o
